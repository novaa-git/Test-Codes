...
public class ecgimage {
    ...
    /**
                String dcmFile = files.getImageFolder();
                //dcmFile = "C:\\Users\\serkan.sert\\Desktop\\ECG\\1.3.6.1.4.1.5962.1.09311501.20210311124513.1.4513.1.1.dcm";
                if ((dcmFile != null) && (new File(dcmFile).exists())) {
                    ecgimage tl = new ecgimage();
                        tl.setFileName(dcmFile);
                        String imageFormat = request.getParameter("type");
                        if (imageFormat != null) {
                            tl.setExportImageFormat(imageFormat);    
                        }
                        String patientinfo = request.getParameter("info");
                        if ((patientinfo != null) && patientinfo.equals("H"))
                            tl.setIsPatientInfo(false);
                            
                        String width = request.getParameter("width");
                        String height = request.getParameter("height");
                        if (ConverterUtils.StrToInt(width) == 0)
                            width = "1200";
                        if (ConverterUtils.StrToInt(height) == 0)
                            height = "900";
                                
                        tl.setWidth(ConverterUtils.StrToInt(width));
                        tl.setHeight(ConverterUtils.StrToInt(height));
                        
                        String format = request.getParameter("format");
                        if (format == null)
                            format = EcgParams.DEFAULTFORMAT;
                        else
                            if (format.equals("1"))
                                format = EcgParams.DEFAULTFORMAT;
                                else 
                                if (format.equals("2"))
                                    format = EcgParams.TWOPARTS;
                        tl.setFormat(format);
                        
                    ByteArrayOutputStream res = tl.getEcgToByteArray();
